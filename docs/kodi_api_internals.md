...
17. **Add-on script invocations share ONE OS process** (CPythonInvoker sub-interpreters): `os.getpid()` is identical across overlapping invocations, so it is useless for per-invocation uniqueness (temp names, locks) — use `uuid.uuid4()` or a monotonic counter in a shared store. "Fresh process per search" phrasing elsewhere in the docs means a fresh *interpreter* (module state is NOT shared between invocations), not a fresh PID. Caught by review on repo-plugins PR #4812.
