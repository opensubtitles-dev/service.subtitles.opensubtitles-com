...
def clear_cache():
    cache = Cache("OpenSubtitles")
    items_cleared, bytes_cleared = cache.clear()
    ...
    kb_cleared = round(bytes_cleared / 1024, 2)
    msg = f"Cache cleared: {items_cleared} items ({kb_cleared} KB freed)"
    xbmcgui.Dialog().notification(f"{__addon_name__} v{__version__}", msg, __icon__, 3000, False)
